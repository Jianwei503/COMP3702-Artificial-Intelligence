...
class Node:
    ...
    def __lt__(self, other):
        distance_self = abs(self.map.player_x - self.flag_coordinate[0]) \
                        + abs(self.map.player_y - self.flag_coordinate[1])
        distance_other = abs(other.map.player_x - other.flag_coordinate[0]) \
                         + abs(other.map.player_y - other.flag_coordinate[1])
        return self.steps + distance_self < other.steps + distance_other
...
def main(arglist):
    ...
    init_state.flag_coordinate = find_flag(game_map)
    visited = []
