...
def main(arglist):
    ...
    init_state = Node(coord=(game_map.player_x, game_map.player_y),
        ...
        for move in action_set:
            ...
            if not new_map.apply_move(move): # moved successfully, apply_move() returns 0
                ...
                if child.id not in visited:
                    ...
                    child.total_cost = child.steps + \
                                       abs(child.coord[0] - goal[0]) + abs(child.coord[1] - goal[1])
                    fringe.put(child)
                    path[child.id] = path[parent.id] + [move]
            del new_map
