...
def main(arglist):
    ...
    if game_map.method == 'q-learning':
        ...
        if not WINDOWS and not DEBUG_MODE:
            signal.alarm(0)
#############################################################################
    # stat = game_map.make_clone()
    # stat.player_x = 2
    # stat.player_y = 1
    # stat.player_heading = 0
    # a = solver.get_policy(stat)
    #
    # stat.player_x = 2
    # stat.player_y = 1
    # stat.player_heading = 3
    # b = solver.get_policy(stat)
    #
    # stat.player_x = 3
    # stat.player_y = 1
    # stat.player_heading = 3
    # c = solver.get_policy(stat)
#############################################################################
    ...
    print(f"Avg Episode Reward = {str(round(total_reward, 2))}")
